...
void RepastHPCDemoAgent::processValues(repast::ValueLayerND<double>* valueLayer, repast::SharedDiscreteSpace<RepastHPCDemoAgent, repast::WrapAroundBorders, repast::SimpleAdder<RepastHPCDemoAgent> >* space){
    std::vector<int> agentLoc;
    space->getLocation(id_, agentLoc);
    ...
    if(originalValue < nextValue && total > 0){                // If the likely next value is better
      valueLayer->addValueAt(total-c, agentLoc, errorFlag1);   // Drop part of what you're carrying
        std::cout << "Agent " << id_ << " dropping " << (total - c) << " at " << agentLoc[0] << "," << agentLoc[1] << "," << agentLoc[2] << " for new value " << valueLayer->getValueAt(agentLoc, errorFlag1) << std::endl;
      total = c;
      ...
      valueLayer->setValueAt(0, agentLoc, errorFlag2);  // Value Layer is now at zero
        std::cout << "Agent " << id_ << " picking up " << originalValue << " at " << agentLoc[0] << "," << agentLoc[1] << "," << agentLoc[2] << " for new value " << valueLayer->getValueAt(agentLoc, errorFlag2)<< std::endl;
    }
}
